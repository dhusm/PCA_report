...
for i,kT in enumerate(kT_list):
    ...
    df.loc[i] = [kT/kb*1e9,sm,mu,np.array(nOD,dtype='float'),Ntot,EE0,Ef,ttf]
...
X,Y = np.meshgrid(y_sample*1e6,range(len(vecs)))
## im_a2 = a2.imshow(vecs,cmap=cm.RdBu)
im_a2 = a2.pcolormesh(X,Y,vecs,cmap=cm.RdBu)
im_a2.set_clim([-0.4,0.4])
a2.set_ylabel('PC rank')
colorbar(im_a2,ax=a2)
X,Y = np.meshgrid(y_sample*1e6,range(10))
## im_aa2 = aa2.imshow(vecs[:10],aspect='auto',cmap=cm.RdBu)
im_aa2 = aa2.pcolormesh(X,Y,vecs[:10],cmap=cm.RdBu)
im_aa2.set_clim([-0.4,0.4])
...
for n,nOD in enumerate(nOD_list):
    for i in range(len(my_pca.val)):
        ...
        sm = np.sum((y_sample/px)**2*rec_pic)/np.sum(rec_pic)
        sm_rec[n,i] = sm
